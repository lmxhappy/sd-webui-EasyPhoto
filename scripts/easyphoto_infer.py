...
from scripts.easyphoto_config import user_id_outpath_samples, easyphoto_outpath_samples, validation_prompt, \
    DEFAULT_POSITIVE, DEFAULT_NEGATIVE, easyphoto_img2img_samples
...
def test(user_ids, best_lora_weights, additional_prompt, multi_user_facecrop_ratio):
    ...
    for user_id in user_ids:
        ...
        _face_id_retinaface_boxes, _face_id_retinaface_keypoints, _face_id_retinaface_masks = call_face_crop(
            retinaface_detection, face_id_image, multi_user_facecrop_ratio, "roop")
    ...
    return input_prompts, face_id_images, roop_images, face_id_retinaface_boxes, face_id_retinaface_keypoints, face_id_retinaface_masks


def test2(second_denoising_strength, input_prompt_without_lora, color_shift_last, second_diffusion_steps,
          after_face_fusion_ratio, color_shift_middle, seed, first_denoising_strength, first_diffusion_steps,
          before_face_fusion_ratio, apply_face_fusion_before, template_images, multi_user_safecrop_ratio, tabs,
          user_ids, crop_face_preprocess, input_prompts, face_id_images, roop_images, face_id_retinaface_boxes,
          face_id_retinaface_keypoints, face_id_retinaface_masks, apply_face_fusion_after):
    outputs = []
    for template_image in template_images:
        ...
        template_face_safe_boxes, _, _ = call_face_crop(retinaface_detection, template_image, multi_user_safecrop_ratio,
                                                        "crop")
        ...
        if min(len(template_face_safe_boxes), len(user_ids)) > 1:
            ...
            output_mask = Image.fromarray(np.uint8(
                cv2.dilate(np.array(output_mask), np.ones((64, 64), np.uint8), iterations=1) - cv2.erode(
        ...
        for index in range(min(len(template_face_safe_boxes), len(user_ids))):
            ...
            replaced_input_image = crop_and_paste(face_id_images[index], face_id_retinaface_masks[index], input_image,
                                                  face_id_retinaface_keypoints[index], input_image_retinaface_keypoint,
            ...
            if roop_images[index] is not None and apply_face_fusion_before:
                ...
                fusion_image = image_face_fusion(dict(template=input_image, user=roop_images[index]))[
                    OutputKeys.OUTPUT_IMG]  # swap_face(target_img=output_image, source_img=roop_image, model="inswapper_128.onnx", upscale_options=UpscaleOptions())
                ...
                input_image = Image.fromarray(np.uint8((np.array(input_image, np.float32) * (
                        1 - before_face_fusion_ratio) + np.array(fusion_image,
                                                                 np.float32) * before_face_fusion_ratio)))
            ...
            input_image_retinaface_box[0] = np.clip(
                np.array(input_image_retinaface_box[0], np.int32) - face_width * 0.15, 0, w - 1)
            ...
            first_diffusion_output_image = inpaint_with_mask_face(input_image, input_mask, replaced_input_image,
                                                                  diffusion_steps=first_diffusion_steps,
                                                                  denoising_strength=first_denoising_strength,
                                                                  input_prompt=input_prompts[index], hr_scale=1.0,
                                                                  seed=str(seed))

            if color_shift_middle:
                # apply color shift
                first_diffusion_output_image_face_area = np.array(copy.deepcopy(first_diffusion_output_image))[
                                                         input_image_retinaface_box[1]:input_image_retinaface_box[3],
                                                         input_image_retinaface_box[0]:input_image_retinaface_box[2], :]
                first_diffusion_output_image_face_area = color_transfer(first_diffusion_output_image_face_area,
                                                                        template_image_original_face_area)

                first_diffusion_output_image = np.array(first_diffusion_output_image)
                first_diffusion_output_image[input_image_retinaface_box[1]:input_image_retinaface_box[3],
                input_image_retinaface_box[0]:input_image_retinaface_box[2], :] = first_diffusion_output_image_face_area
                first_diffusion_output_image = Image.fromarray(first_diffusion_output_image)

            # Obtain the mask of the area around the face
            input_mask = Image.fromarray(np.uint8(
                cv2.dilate(np.array(origin_input_mask), np.ones((96, 96), np.uint8), iterations=1) - cv2.erode(
                    np.array(origin_input_mask), np.ones((48, 48), np.uint8), iterations=1)))
            ...
            if roop_images[index] is not None and apply_face_fusion_after:
                ...
                input_image = Image.fromarray(np.uint8((np.array(first_diffusion_output_image, np.float32) * (
                        1 - after_face_fusion_ratio) + np.array(fusion_image,
                                                                np.float32) * after_face_fusion_ratio)))
            ...
            second_diffusion_output_image = inpaint_only(input_image, input_mask, input_prompts[index],
                                                         diffusion_steps=second_diffusion_steps,
                                                         denoising_strength=second_denoising_strength,
                                                         fusion_image=fusion_image, hr_scale=default_hr_scale)
            ...
            if color_shift_last:
                ...
                # apply color shift
                second_diffusion_output_image_face_area = np.array(copy.deepcopy(second_diffusion_output_image))[
                                                          rescale_retinaface_box[1]:rescale_retinaface_box[3],
                                                          rescale_retinaface_box[0]:rescale_retinaface_box[2], :]
                second_diffusion_output_image_face_area = color_transfer(second_diffusion_output_image_face_area,
                                                                         template_image_original_face_area)

                second_diffusion_output_image = np.array(second_diffusion_output_image)
                second_diffusion_output_image[rescale_retinaface_box[1]:rescale_retinaface_box[3],
                rescale_retinaface_box[0]:rescale_retinaface_box[2], :] = second_diffusion_output_image_face_area
            ...
            if min(len(template_face_safe_boxes), len(user_ids)) > 1:
                ...
                output_image[template_face_safe_box[1]:template_face_safe_box[3],
                template_face_safe_box[0]:template_face_safe_box[2]] = np.array(loop_output_image, np.float32)[
                                                                       template_face_safe_box[1]:template_face_safe_box[
                                                                           3],
                                                                       template_face_safe_box[0]:template_face_safe_box[
                                                                           2]]
            else:
        ...
        if min(len(template_face_safe_boxes), len(user_ids)) > 1:
            ...
            output_image = inpaint_only(output_image, output_mask, input_prompt_without_lora, diffusion_steps=20,
                                        denoising_strength=0.3, hr_scale=1)
        ...
        save_image(output_image, easyphoto_outpath_samples, "EasyPhoto", None, None, opts.grid_format, info=None,
                   short_filename=not opts.grid_extended_filename, grid=True, p=None)
...
def easyphoto_infer_forward(
        ...
        color_shift_last, tabs, *user_ids
):
    ...
    input_prompts, face_id_images, roop_images, face_id_retinaface_boxes, face_id_retinaface_keypoints, face_id_retinaface_masks = test(
        user_ids, best_lora_weights, additional_prompt, multi_user_facecrop_ratio)

    outputs = test2(second_denoising_strength, input_prompt_without_lora, color_shift_last, second_diffusion_steps,
                    after_face_fusion_ratio, color_shift_middle, seed, first_denoising_strength, first_diffusion_steps,
                    before_face_fusion_ratio, apply_face_fusion_before, template_images, multi_user_safecrop_ratio,
                    tabs,
                    user_ids, crop_face_preprocess, input_prompts, face_id_images, roop_images,
                    face_id_retinaface_boxes,
                    face_id_retinaface_keypoints, face_id_retinaface_masks, apply_face_fusion_after)
